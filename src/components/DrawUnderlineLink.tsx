...
const svgVariants = [
  ...
  `<svg width="310" height="40" viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 29.8857C52.3147 26.9322 99.4329 21.6611 146.503 17.1765C151.753 16.6763 157.115 15.9505 162.415 15.6551C163.28 15.6069 165.074 15.4123 164.383 16.4275C161.704 20.3627 157.134 23.7551 153.95 27.4983C153.209 28.3702 148.194 33.4751 150.669 34.6605C153.638 36.0819 163.621 32.6063 165.039 32.2029C178.55 28.3608 191.49 23.5968 204.869 19.5404C231.903 11.3436 259.347 5.83254 288.793 5.12258C294.094 4.99476 299.722 4.82265 305 5.45025" stroke="#DDA15E" stroke-width="10" stroke-linecap="round"/></svg>`
];

function getNextIndex(current: number | null) {
  if (current === null) return 0;
  return (current + 1) % svgVariants.length;
}

function getSVGandPath(svgString: string): { svg: SVGSVGElement; path: SVGPathElement } | null {
  const temp = document.createElement("div");
  temp.innerHTML = svgString;
  const svg = temp.querySelector("svg");
  const path = temp.querySelector("path");
  if (svg && path) return { svg, path };
  return null;
}

type AnchorProps = React.AnchorHTMLAttributes<HTMLAnchorElement>;
type DrawUnderlineLinkProps = React.PropsWithChildren<{
  className?: string;
  target?: AnchorProps["target"];
  rel?: AnchorProps["rel"];
}> & Omit<LinkProps, "passHref"> & Omit<AnchorProps, "href" | "className" | "children" | "target" | "rel">;

const DrawUnderlineLink: React.FC<DrawUnderlineLinkProps> = ({
  children,
  className = "",
  target,
  rel,
  ...
  const nextIndex = useRef<number>(Math.floor(Math.random() * svgVariants.length));
  ...
  const handleEnter = useCallback(() => {
    ...
    if (leaveTween.current && leaveTween.current.isActive()) leaveTween.current.kill();
    setSvgIndex(nextIndex.current);
    nextIndex.current = (nextIndex.current + 1) % svgVariants.length;
  }, []);

  // Animate in when svgIndex changes
  React.useEffect(() => {
    if (svgIndex === null || !svgBoxRef.current) return;
    svgBoxRef.current.innerHTML = svgVariants[svgIndex];
    const svg = svgBoxRef.current.querySelector("svg");
    const path = svgBoxRef.current.querySelector("path");
    if (svg && path) {
      svg.setAttribute("class", "text-draw__box-svg");
  ...
  const handleLeave = useCallback(() => {
    ...
    // Animate out: left-to-right disappearance
    leaveTween.current = gsap.to(path, {
      ...
      onComplete: () => {
        ...
        if (svgBoxRef.current) svgBoxRef.current.innerHTML = "";
        setSvgIndex(null);
      }
    });
  }, []);
...
export default DrawUnderlineLink;
